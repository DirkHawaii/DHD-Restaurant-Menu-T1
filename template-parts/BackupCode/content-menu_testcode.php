...
if ( $menu_tabs->have_posts() ) :
  ...
  while ( $menu_tabs->have_posts() ) :  // TAB LOOP
    ...
    //$sql_calls_str .= $menu_tabs->request .'<br/>';
    ...
    if ( $tab_count == 1 ) {
      $tab_htm_content .= '<h2>'. $menu_tab_title .'</h2>';
    } else {
      $tab_htm_content .= '<h2>'. $menu_tab_title .'</h2>';
    }
    ...
    if ( $menu_sections->have_posts() ) :
      ...
      while ( $menu_sections->have_posts() ) :  // SECTION LOOP
        ...
        if ($section_count == 1) {
          $sql_calls_str .= $menu_sections->request .'<br/>';
        ...
        if ( $current_column != $menu_section_column ) { // COLUMN HAS CHANGED
          $current_column = intval($menu_section_column);
          $tab_htm_content .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Column Change</b><br/>';
        }

        $tab_htm_content .= '&nbsp;&nbsp;<b>'. $menu_section_title .' N:'. $section_count .' C:'. $menu_section_column .' R:'. $menu_section_row .'</b><br/>';
        ...
        if ( $menu_items->have_posts() ) :
          while ( $menu_items->have_posts() ) :  // ITEM LOOP
            ...
            if ( strpos( $menu_item_title, '|' ) === false ) {  // IT IS NOT A LIST OF TITLES
              $price_str = $menu_item_title .' - '. $menu_item_price .'<br/>';
              ...
              for ($i = 0; $i < count($item_title_ar); ++$i) {        // LOOP THROUGH ARRAY OF TITLES
                ...
                if ( strpos( $item_price_ar[$i], ',' ) === false ) {                                          /***   S I N G L E   P R I C E   ***/
                  ...
                  if ($dot_lead) {
                    $price_str .= $item_title_ar[$i] .' - '. $item_price_str .'<br/>';
                  } else {
                    $price_str .= $item_title_ar[$i] .' - '. $item_price_str .'<br/>';
                  }
                  ...
                  if ($dot_lead) {
                    $price_str .= $item_title_ar[$i] .' - '. $partial_price_str .'<br/>';
                  } else {
                    $price_str .= $item_title_ar[$i] .' - '. $partial_price_str .'<br/>';
                  }
                }
            ...
            //$tab_htm_content .= '&nbsp;&nbsp;&nbsp;&nbsp;'. get_the_title() .'<br/>';
